  ...
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      ...
      return res.status(500).json({
        error: 'Payment succeeded, but we could not prepare your access automatically. Please contact hello@r0ready.com.',
      });
    }
    ...
    try {
      ...
      // Best-effort conversion tracking for free-trial leads.
      try {
        await fetch(
          supabaseUrl + '/rest/v1/demo_leads?email=eq.' + encodeURIComponent(email.toLowerCase()),
          {
            method: 'PATCH',
            headers: {
              apikey: supabaseKey,
        ...
        console.error('[demo-lead-convert]', leadErr.message);
      }
    } catch (supabaseErr) {
    ...
    const fromEmail = process.env.FROM_EMAIL || 'R0Ready <hello@r0ready.com>';
    ...
    if (!emailRes.ok) {
      ...
      return res.status(200).json({
        ...
        product: config.product,
        emailSent: false,
        warning: 'Payment confirmed. Your email did not send, so open the app below and bookmark that link. If you need help, contact hello@r0ready.com.',
    ...
    return res.status(500).json({ error: 'Internal error. Contact hello@r0ready.com' });
  }
};
